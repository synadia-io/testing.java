...
public class CommandLine {
    ...
    public final String crWorkDirectory;
    public final long crInitialDelay;
    public final long crDelay;
    public final long crDownTime;
    public final boolean crRandom;

    public Options makeManagmentOptions(String label) {
        return makeOptions(new OutputConnectionListener(label), new OutputErrorListener(label), 0);
    }
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder("HealthChecker Test App Config ");
        ...
        append(sb, "crServers", crServers, true);
        append(sb, "crWorkDirectory", crWorkDirectory, crWorkDirectory != null);
        append(sb, "crInitialDelay", crInitialDelay, true);
        append(sb, "crDelay", crDelay, true);
        append(sb, "crDownTime", crDownTime, true);
        append(sb, "crRandom", crRandom, crRandom);
    ...
    public CommandLine(String[] args) {
        try {
            ...
            long _crInitialDelay = 60000;
            long _crDelay = 60000;
            long _crDownTime = 60000;
            boolean _crRandom = false;
                    ...
                    for (int x = 0; x < args.length; x++) {
                        ...
                        switch (arg) {
                            ...
                            case "--push":
                                ...
                                break;
                            case "--crservers":
                                _crServers = Integer.parseInt(args[++x]);
                                break;
                            case "--crdir":
                                _crWorkDirectory = args[++x];
                                break;
                            case "--crinitial":
                                _crInitialDelay = Long.parseLong(args[++x]);
                                break;
                            case "--crdelay":
                                _crDelay = Long.parseLong(args[++x]);
                                break;
                            case "--down":
                                _crDownTime = Long.parseLong(args[++x]);
                                break;
                            case "--crrandom":
                                _crRandom = true;
                                break;
...
//                    e.printStackTrace();
            ...
            crWorkDirectory = _crWorkDirectory;
            crInitialDelay = _crInitialDelay;
            crDelay = _crDelay;
            crDownTime = _crDownTime;
            crRandom = _crRandom;
        }
